...
if sys.version_info >= (3, 10):
    ...
    UnionType = typing._GenericAlias
...
def format_destination(
    src: str, config: Optional[types.CreateBatchJobConfigOrDict] = None
) -> types.CreateBatchJobConfig:
    """Formats the destination uri based on the source uri."""
    config = types._CreateBatchJobParameters(config=config).config or types.CreateBatchJobConfig()
...
def get_function_map(
    ...
    config_model = (
        types.GenerateContentConfig(**config) if config and isinstance(config, dict) else config
    )
...
def convert_number_values_for_function_call_args(
    ...
    if isinstance(args, dict):
        return {
            key: convert_number_values_for_function_call_args(value) for key, value in args.items()
        }
    ...
    if get_origin(annotation) in (Union, UnionType):
        for arg in get_args(annotation):
            if (
                ...
                try:
                    return convert_if_exist_pydantic_model(value, arg, param_name, func_name)
    ...
    for param_name, param in signature.parameters.items():
        if param_name in args:
            converted_args[param_name] = convert_if_exist_pydantic_model(
                args[param_name], param.annotation, param_name, func_name
            )
    try:
...
def get_function_response_parts(
    response: types.GenerateContentResponse, function_map: Dict[str, object]
) -> List[types.Part]:
...
def should_disable_afc(config: Optional[types.GenerateContentConfigOrDict] = None) -> bool:
    ...
    if (
        ...
        and "maximum_remote_calls" in config_model.automatic_function_calling.model_fields_set
        and int(config_model.automatic_function_calling.maximum_remote_calls) > 0
    ):
        logger.warning(
...
def get_max_remote_calls_afc(config: Optional[types.GenerateContentConfigOrDict] = None) -> int:
    """Returns the remaining remote calls for automatic function calling."""
    if should_disable_afc(config):
        raise ValueError(
            "automatic function calling is not enabled, but SDK is trying to get max remote calls."
        )
    config_model = (
        types.GenerateContentConfig(**config) if config and isinstance(config, dict) else config
    )
...
def should_append_afc_history(config: Optional[types.GenerateContentConfigOrDict] = None) -> bool:
    config_model = (
        types.GenerateContentConfig(**config) if config and isinstance(config, dict) else config
    )

...
def test_factory_method_from_uri_part():
    ...
    assert my_part.file_data.file_uri == "gs://generativeai-downloads/images/scones.jpg"
...
def test_factory_method_from_executable_code_part():
    my_part = SubPart.from_executable_code(code='print("hello")', language="PYTHON")
...
def test_generic_alias_array():
    ...
    expected_schema_mldev = types.FunctionDeclaration(
        ...
        parameters=types.Schema(
            type="OBJECT",
            properties={
                "a": types.Schema(type="ARRAY", items=types.Schema(type="INTEGER")),
            },
        ),
...
def test_function_with_option_vertex(monkeypatch):
    ...
    actual_schema_vertex = types.FunctionDeclaration.from_callable_with_api_option(

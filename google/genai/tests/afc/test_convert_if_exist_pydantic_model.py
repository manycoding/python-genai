...
def test_builtin_types():
    ...
    assert convert_if_exist_pydantic_model(1.0, float, "param_name", "func_name") == 1.0
    assert (
        convert_if_exist_pydantic_model("1.0", str, "param_name", "func_name") == "1.0"
    )
    assert (
        convert_if_exist_pydantic_model(True, bool, "param_name", "func_name") is True
    )
    assert convert_if_exist_pydantic_model([1], list, "param_name", "func_name") == [1]
...
def test_value_int_annotation_float():
    assert convert_if_exist_pydantic_model(1, float, "param_name", "func_name") == 1.0


def test_union_types():
    assert (
        convert_if_exist_pydantic_model(1, Union[int, float], "param_name", "func_name")
        == 1
    )
    assert (

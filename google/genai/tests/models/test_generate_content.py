...
test_table: List[pytest_helper.TestTableItem] = [
    ...
    pytest_helper.TestTableItem(
        ...
        parameters=types._GenerateContentParameters(
            ...
            config={"system_instruction": t.t_content(None, "I say high, you say low")},
        ),
    ),
    pytest_helper.TestTableItem(
        name="test_labels",
    ...
    pytest_helper.TestTableItem(
        ...
        parameters=types._GenerateContentParameters(
            model="gemini-2.0-flash-exp",
            contents=t.t_contents(None, 'Produce a speech response saying "Cheese"'),
            config=types.GenerateContentConfig(
...
def test_pydantic_schema_with_optional_generic_alias(client):
    ...
    assert isinstance(response.parsed, CountryInfo)
    assert (
        isinstance(response.parsed.official_languages, list)
        or response.parsed.official_languages is None
    )
                ...
                items=types.Schema(
                    any_of=[
                        types.Schema(
                            ...
                            properties={
                                "type": types.Schema(
                                    type=types.Type.STRING, description='Always "apple"'
                                ),
...
def test_function(client):
    ...
    response = client.models.generate_content(
        model="gemini-1.5-flash",
        contents=(
            "What is the weather like in Sunnyvale? Answer in very short sentence."
        ),
        config={
            "tools": [get_weather],
        },

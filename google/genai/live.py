...
from . import _api_module, _common, client, errors, types
...
class AsyncLive(_api_module.BaseModule):
    ...
    def _LiveSetup_to_mldev(
        ...
        if getv(config, ["system_instruction"]) is not None:
            setv(
                ...
                _Content_to_mldev(
                    self._api_client,
                    t.t_content(self._api_client, getv(config, ["system_instruction"])),
                    to_object,
                ),
            )
    ...
    async def connect(
        ...
        *,
        ...
        parameter_model = (
            types.LiveConnectConfig(**config) if isinstance(config, dict) else config
        )

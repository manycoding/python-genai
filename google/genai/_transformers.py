...
if sys.version_info >= (3, 10):
    ...
    VersionedUnionType = typing._GenericAlias
...
def _resource_name(
    ...
    *,
    ...
    should_prepend_collection_identifier = (
        ...
        and f"{collection_identifier}/{resource_name}".count("/") + 1 == collection_hierarchy_depth
    )
    if client.vertexai:
...
def t_extract_models(api_client: _api_client.ApiClient, response: dict) -> List[types.Model]:
    ...
    elif response.get("httpHeaders") is not None and response.get("jsonPayload") is None:
        return []
    else:
...
def t_parts(client: _api_client.ApiClient, parts: Union[list, PartType]) -> List[types.Part]:
    if not parts:
...
def t_image_predictions(client: _api_client.ApiClient, predictions) -> List[types.GeneratedImage]:
    if not predictions:
        return None
    images = []
...
def t_content(client: _api_client.ApiClient, content: ContentType):
    if not content:
...
def _process_enum(
    enum: EnumMeta,
    client: Optional[_api_client.ApiClient] = None,  # type: ignore
) -> types.Schema:
    for member in enum:  # type: ignore
        if not isinstance(member.value, str):
            raise TypeError(
                f"Enum member {member.name} value must be a string, got {type(member.value)}"
            )
...
def t_schema(
    ...
    if (
        ...
        not isinstance(origin, typing._GenericAlias)
        ...
        isinstance(origin, typing._GenericAlias)
        ...
        return types.SpeechConfig(
            voice_config=types.VoiceConfig(
                prebuilt_voice_config=types.PrebuiltVoiceConfig(
                    voice_name=origin["voice_config"]["prebuilt_voice_config"].get("voice_name")
                )
            )
        )
...
def t_tools(client: _api_client.ApiClient, origin: List[Any]) -> List[types.Tool]:
    if not origin:
        return []
    ...
    for tool in origin:
        ...
        if transformed_tool.function_declarations:
            function_tool.function_declarations += transformed_tool.function_declarations
        else:
...
def t_batch_job_source(client: _api_client.ApiClient, src: str):
    if src.startswith("gs://"):
        return types.BatchJobSource(format="jsonl", gcs_uri=[src])
    elif src.startswith("bq://"):
        return types.BatchJobSource(format="bigquery", bigquery_uri=src)
    else:
...
def t_batch_job_destination(client: _api_client.ApiClient, dest: str):
    if dest.startswith("gs://"):
        return types.BatchJobDestination(format="jsonl", gcs_uri=dest)
    elif dest.startswith("bq://"):
        return types.BatchJobDestination(format="bigquery", bigquery_uri=dest)
    else:
...
def t_resolve_operation(api_client: _api_client.ApiClient, struct: dict):
    if (name := struct.get("name")) and "/operations/" in name:
        ...
        while operation.get("done") != True:
            ...
            operation = api_client.request(http_method="GET", path=name, request_dict={})
            time.sleep(delay_seconds)
            total_seconds += total_seconds
            # Exponential backoff
            delay_seconds = min(
                delay_seconds * LRO_POLLING_MULTIPLIER, LRO_POLLING_MAXIMUM_DELAY_SECONDS
            )
        if error := operation.get("error"):
            raise RuntimeError(f"Operation {name} failed with error: {error}.\n{operation}")
        return operation.get("response")
    else:
        return struct


def t_file_name(api_client: _api_client.ApiClient, name: Optional[Union[str, types.File]]):
...
def t_tuning_job_status(api_client: _api_client.ApiClient, status: str) -> types.JobState:

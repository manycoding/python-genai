...
from typing import Any, AsyncIterator, Dict, List, Optional, Tuple, TypedDict, Union
...
def _append_library_version_headers(headers: Dict[str, str]) -> None:
    ...
    if "x-goog-api-client" in headers and version_header_value not in headers["x-goog-api-client"]:
...
def _patch_http_options(options: HttpOptionsDict, patch_options: Dict[str, Any]) -> HttpOptionsDict:
    ...
    for patch_key, patch_value in patch_options.items():
        ...
        if isinstance(patch_value, dict) and isinstance(copy_option.get(patch_key, None), dict):
            copy_option[patch_key] = {}
            copy_option[patch_key].update(options[patch_key])  # shallow copy from original options.
...
def _load_auth(*, project: Union[str, None]) -> Tuple[Credentials, str]:
    ...
    if not project:
        raise ValueError("Could not resolve project using application default credentials.")
...
class ApiClient:
    ...
    def __init__(
        ...
        if self.vertexai:
            ...
            if not ((self.project and self.location) or self.api_key):
                raise ValueError(
                    "Project and location or API key must be set when using the Vertex AI API."
                )
    ...
    def _build_request(
        ...
        if http_options:
            if isinstance(http_options, HttpOptions):
                ...
                patched_http_options = _patch_http_options(self._http_options, http_options)
        else:
        ...
        if self.vertexai and http_method == "get" and path.startswith("publishers/google/models"):
        ...
        url = _join_url_path(
            patched_http_options["base_url"], patched_http_options["api_version"] + "/" + path
        )
    ...
    def _request(self, http_request: HttpRequest, stream: bool = False) -> HttpResponse:
        if self.vertexai and not self.api_key:
            ...
            if self._credentials.quota_project_id:
                http_request.headers["x-goog-user-project"] = self._credentials.quota_project_id
    ...
    async def _async_request(self, http_request: HttpRequest, stream: bool = False):
        if self.vertexai:
        ...
        if stream:
            ...
            response = await aclient.send(httpx_request, stream=stream)
            errors.APIError.raise_for_response(response)
            return HttpResponse(response.headers, response if stream else [response.text])
        else:
    ...
    def request(
        ...
        http_request = self._build_request(http_method, path, request_dict, http_options)
        response = self._request(http_request, stream=False)
        json_response = response.json
        if not json_response:
            return BaseResponse(http_headers=response.headers).model_dump(by_alias=True)
    ...
    def _upload_fd(self, file: io.IOBase, upload_url: str, upload_size: int) -> str:
        """Transfers a file to the given URL.
    ...
    def _download_file_request(self, http_request: HttpRequest) -> HttpResponse:
    ...
    async def async_upload_file(
        ...
        returns:
              The response json object from the finalize request.
        """
        return await asyncio.to_thread(self.upload_file, file_path, upload_url, upload_size)

    async def _async_upload_fd(self, file: io.IOBase, upload_url: str, upload_size: int) -> str:
        """Transfers a file asynchronously to the given URL.
        ...
        returns:
              The response json object from the finalize request.
        """
        return await asyncio.to_thread(self._upload_fd, file, upload_url, upload_size)
    ...
    async def async_download_file(self, path: str, http_options):
        ...
        returns:
              The file bytes
        """
        return await asyncio.to_thread(self.download_file, path, http_options)
